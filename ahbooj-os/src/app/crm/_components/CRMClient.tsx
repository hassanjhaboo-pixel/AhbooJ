...
import { Plus, Search, Users, MessageCircle, Mail, Trophy } from 'lucide-react'
...
const PAGE_TABS = [
  ...
  { value: 'referrals',   label: 'Referrals', icon: Trophy },
]
...
export function CRMClient({
  ...
  return (
    <>
      ...
      {/* Page tabs */}
      <div className="flex items-center gap-1 mb-5 border-b border-espresso/10 pb-0">
        {PAGE_TABS.map(tab => {
          const Icon = tab.icon
          return (
            <button
            ...
            >
              <Icon size={14} />
              {tab.label}
              {tab.value === 'referrals' && referrers.length > 0 && (
                <span className="ml-1 bg-terracotta/10 text-terracotta text-xs px-1.5 py-0.5 rounded-full">
                  {referrers.length}
                </span>
              )}
            </button>
      ...
      {pageTab === 'referrals' ? (
        <ReferralLeaderboard referrers={referrers} />
      ) : (
        /* Search + filter */
