...
interface IngredientRow {
  key: number
  recipe_ingredient_id: string | null  // null for newly added rows
...
export function RecipeEditForm({ recipe, existingIngredients, allIngredients }: RecipeEditFormProps) {
  const router = useRouter()
  const supabase = createClient()

  const [name,         setName]         = useState(recipe.name)
  const [category,     setCategory]     = useState(recipe.category ?? '')
  const [yieldUnits,   setYieldUnits]   = useState(String(recipe.base_yield_units))
  const [yieldLabel,   setYieldLabel]   = useState(recipe.yield_unit_label)
  ...
  const [rows, setRows] = useState<IngredientRow[]>(
    existingIngredients.length > 0
      ? existingIngredients.map((ei, idx) => ({
          key:                  idx,
          recipe_ingredient_id: ei.id,
          ingredient_id:        ei.ingredient_id,
          quantity:             String(ei.quantity),
          unit:                 ei.unit,
          notes:                ei.notes ?? '',
        }))
      : [{ key: 0, recipe_ingredient_id: null, ingredient_id: '', quantity: '', unit: '', notes: '' }]
  )

  let nextKey = rows.length

  function addRow() {
    setRows(r => [...r, { key: nextKey++, recipe_ingredient_id: null, ingredient_id: '', quantity: '', unit: '', notes: '' }])
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      // 2. Replace all recipe_ingredients (delete + re-insert)
